...
def _(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        env_var="URL",
        default="https://oaktrust-pre.library.tamu.edu",
        help="Base URL",
    )
...
def log_request(
    ...
    **kwargs,
    ...
    try:
        if exception:
            logger.error(
                f"FAILURE: {request_type} {name} {response_time} {exception} at {start_time} for {url}"
            )
        elif response_time > 20000:
            logger.warning(
                f"Long Request: {name} took {response_time} for {url} at {start_time}"
            )
        elif response_time > 1000:
            logger.warning(
                f"At least a second: {name} took {response_time} for {url} at {start_time}"
            )
        else:
...
class OakTrustTester(FastHttpUser):
    ...
    def get_collections(self):
        response = self.client.get(
            f"{self.host}/server/api/core/collections",
            name="Get Collection Data from API",
        )
        ...
        current_collections = (
            self.client.get(
                f"{self.host}/server/api/core/collections?page={current}size=1",
                name="Get Random List of Collections from API",
            )
            .json()
            .get("_embedded")
            .get("collections")
        ...
        for collection in current_collections:
            ...
            self.client.get(
                f"{self.host}/collections/{uuid}", name="Get Collection by UUID"
            )
        return
    ...
    def lookup_authors(self):
        ...
        author_search = self.client.get(
            rand_author["_links"]["search"]["href"],
            name="Get Random Author Articles from API",
        )
        for article in author_search.json()["_embedded"]["searchResult"]["_embedded"][
            "objects"
        ]:
            self.client.get(
                f"{self.host}/items/{article["_embedded"]["indexableObject"]["uuid"]}",
    ...
    def download_bitstreams(self):
        ...
        all_article_response = author_search.json()["_embedded"]["searchResult"][
            "_embedded"
        ]["objects"]
        all_articles = [
            item["_links"]["indexableObject"]["href"] for item in all_article_response
        ]
        random_article = random.choice(all_articles)
        article_response = self.client.get(
            random_article, name="Get Article API Response by UUID"
        ).json()
        bundles = self.client.get(
            article_response.get("_links")["bundles"]["href"],
            name="Get Bundle API Response",
        )
        ...
        random_bitstream = random.choice(
            random_bundle_response["_embedded"]["bitstreams"]
        )
        self.client.get(
            f"{self.host}/bitstreams/{random_bitstream['uuid']}/download",
            name="Download Random Bitstream",
        )
